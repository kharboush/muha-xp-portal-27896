...
// Calculate mIRC dimensions: maintain aspect ratio (680/646)
const mircAspectRatio = 680 / 646;
const maxMircWidth = 646;
const mircWidth = Math.min(Math.floor(window.innerWidth * 0.9), maxMircWidth);
  ...
  switch (action.type) {
    case ADD_APP: {
      ...
      if (action.payload === 'MediaPlayer') {
        ...
        const mircAspectRatio = 680 / 646;
        const maxWidth = 646;
        windowWidth = Math.min(Math.floor(window.innerWidth * 0.9), maxWidth);
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      if (mircWindow && !mircWindow.maximized) {
        const mircAspectRatio = 680 / 646;
        const maxWidth = 646;
        const mircWidth = Math.min(Math.floor(window.innerWidth * 0.9), maxWidth);
        const mircHeight = Math.floor(mircWidth * mircAspectRatio);
        const mircX = Math.floor((window.innerWidth - mircWidth) / 2);
        const mircY = 60;
